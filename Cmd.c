...
VOID SAVENODES(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD)
 {
  SaveNodes();
...
VOID CMDS00(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD) {
  ...
  Bufferptr = Cmdprintf(Session, Bufferptr, "%s)\r", SESSIONHDDR);
...
VOID CMDP00(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD) {
  ...
  if (PORT->PORTTYPE == 0) 
  {
        if (PORT->Handle)  // Serial port Open
          strcpy(Status, "Open  ");
        else
...
VOID LINKCMD(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD) {
  ...
  if (LINKEDFLAG == 'Y' ||  // UNCONDITIONAL?
      (LINKEDFLAG == 'A' && ((Session->L4CIRCUITTYPE & BPQHOST) || Session->Secure_Session || Session->PASSWORD == 0xffff))) {
    ptr = strtok_s(CmdTail, " ", &Context);
    if (ptr)
      ptr = strtok_s(NULL, " ", &Context);
    ...
    if ((*ptr & 1) == 0) {
      ...
      while ((*ptr & 1) == 0) {
        ...
        if (*ptr & 0x80) {
          ...
          else if ((ptr[7] & 0x80) == 0)  // Repeased by next?
          {
            *(Output++) = '*';  // No, so need *
            Digi = '*';
          }
...
int createandwritefile(char * FN, char * Data, int len);
...
VOID InnerCommandHandler(TRANSPORTENTRY *Session, struct DATAMESSAGE *Buffer) 
{
  ...
  if (CMDPACLEN == 0)
	...
	// if in File Upload add to file buffer

	if (Session->uploadBuffer)
	{
		DIGIMESSAGE Msg;
    ...
    Debugprintf("%s %d", Buffer->L2DATA, Len);
      ...
      Buffer->PID = 0xf0;
      	...
      	ptr1 += sprintf(ptr1, "Cread File failed\r");
			...
			free(Session->uploadFilename);
			Session->uploadFilename = 0;
...
void OpenDirectory();
void GetNextDirEntry(char * line);
...
int picoDelete(char * from);


void DELETECMD (TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD) 
{
  CmdTail = CmdTail + (OrigCmdBuffer - COMMANDBUFFER);  // Replace with original case version

  strlop(CmdTail, ' ');

  if (picoDelete(CmdTail))
    Bufferptr = Cmdprintf(Session, Bufferptr, "Delete OK\r");
  else
    Bufferptr = Cmdprintf(Session, Bufferptr, "Delete failed\r");
            
	SendCommandReply(Session, REPLYBUFFER, (int)(Bufferptr - (char *)REPLYBUFFER));
}

void READCMD (TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD) 
{
  void * file;
  ...
  file = picoOpenFile(CmdTail);

  if (file == 0)
  {  
    Bufferptr = Cmdprintf(Session, Bufferptr, "File %s Not Found\r", CmdTail);
    SendCommandReply(Session, REPLYBUFFER, (int)(Bufferptr - (char *)REPLYBUFFER));    
    return;
  }

  while(GetNextFileBlock(file, line))
  {
    int n = UIRemoveLF(line, strlen(line));
    line[n] = 0;
    Bufferptr = Cmdprintf(Session, Bufferptr, "%s", line); 
  }  
...
VOID RENAMECMD(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *UserCMD)
{
  ...
  if (picoRename(CmdTail, to))
   Bufferptr = Cmdprintf(Session, Bufferptr, "Rename OK\r");
  else
    Bufferptr = Cmdprintf(Session, Bufferptr, "Rename failed\r");

	SendCommandReply(Session, REPLYBUFFER, (int)(Bufferptr - (char *)REPLYBUFFER));
}

void UPLOADCMD(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD)
{
	// Read file from terminal into memory. When /ex received write to file

	CmdTail = CmdTail + (OrigCmdBuffer - COMMANDBUFFER);  // Replace with original case version
	strlop(CmdTail, ' ');

	Session->uploadFilename = _strdup(CmdTail);

	Bufferptr = Cmdprintf(Session, Bufferptr, "Enter file a line at a time. At end enter /ex\r");

	Session->uploadalloc = 2048;	// malloc'ed size
	Session->uploadBuffer = (UCHAR *)malloc(Session->uploadalloc = 2048);
	Session->uploadlen = 0;			// current size
...
VOID FILESCMD(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD) 
{
  char line[256];
...
VOID CLOCKCMD(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *UserCMD)
{
  // Display or update RTC

	struct tm * TM;
	time_t szClock = time(NULL);
  time_t adjust = atoi(CmdTail);
...
VOID QTSMCMD(TRANSPORTENTRY *Session, char *Bufferptr, char *CmdTail, struct CMDX *CMD) {
  ...
  Bufferptr = Cmdprintf(Session, Bufferptr, "Modem %s Centre frequency %d\r",  (KISS->QtSMModem) ? KISS->QtSMModem : "Not Available", KISS->QtSMFreq);
