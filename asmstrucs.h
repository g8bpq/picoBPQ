...
typedef struct _TRANSPORTENTRY
{
	...
	UCHAR * uploadBuffer;		// Used by UPLOAD command
	char * uploadFilename;
	int uploadalloc;			// malloc'ed size
...
typedef struct KISSINFO
{
	...
	int	Alerted;		// Connect Fail Reported
	struct PORTCONTROL * Portvector;
	unsigned char	* RXMSG;				// Msg being built
  unsigned char * RXMPTR;
  int MSGREADY;
  int RXMCNT;					// put pointer for RXMSG
	int	NEEDCRC;				// ETX received, waiting for CRC (NETROM)
...
struct MHTableEntry
{
	...
	char proto;
	short port;
	int	LastHeard;		// Time last packet received
	int Keepalive;
	BOOL IPv6;
};
...
struct CMDX {
  ...
  unsigned char CMDLEN;  // SIGNIFICANT LENGTH
