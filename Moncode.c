...
extern char * strlop(char * buf, char delim);
extern UCHAR * DisplayINP3RIF(UCHAR * ptr1, UCHAR * ptr2, unsigned int msglen);

extern char * DISPLAY_NETROM(MESSAGE * ADJBUFFER, UCHAR * Output, int MsgLen);
extern UCHAR * DISPLAYIPDATAGRAM(IPMSG * IP, UCHAR * Output, int MsgLen);
extern char * DISPLAYARPDATAGRAM(UCHAR * Datagram, UCHAR * Output);


int  SetTraceOptions(int mask, int mtxparam, int mcomparam)
{
